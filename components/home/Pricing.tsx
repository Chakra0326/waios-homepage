...
import { usePricingReveal } from '@/lib/useGsap'
  ...
  return (
    <section className="relative py-24 md:py-32 hairline-b bg-black/20">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div ref={cardsRef} className="mt-20 grid md:grid-cols-3 gap-8 items-stretch">
          {tiers.map((tier) => (
            <div
              key={tier.name}
              data-pricing-card
              data-pricing-highlight={tier.highlight ? 'true' : 'false'}
              className={`relative flex flex-col h-full rounded-3xl p-8 border card-hover ${tier.highlight ? 'border-[#0C1A3E] bg-[#0C1A3E]/20 shadow-[0_0_40px_rgba(12,26,62,0.6)]' : 'hairline bg-[#0A0A0C]'}`}
              style={{ opacity: 0 }}
            >
              {tier.highlight && (
